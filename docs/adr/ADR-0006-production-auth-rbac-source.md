...
- P19 는 `AUTH_RBAC_ENFORCEMENT=1` 일 때 validation route 와 deferred approval route 에 OIDC/JWT 검증과 PLF role lookup 기반 enforcement 를 적용한다.
...
- Validation action 과 deferred approval action 은 unauthorized negative test 와 reviewer spoofing negative test 로 보호한다.
