...
- Canonical role name 은 DDL seed 와 동일한 `USER`, `ADMIN`, `AUDITOR` 로 유지한다.
- `AUTH_RBAC_ENFORCEMENT=1` 일 때 active draft-validation route 에 OIDC/JWT 검증과 PLF role lookup 기반 enforcement 를 적용한다.
...
- Validation action 은 unauthorized negative test 로 보호한다. Decision-gate action 은 product/API surface 에 두지 않는다.
