...
candidate, coordinator handoff, fixture-first demo walkthrough, and blocker triage. P17D
may now report a conditional live pilot release because P17A dependency evidence, P17B
passed artifact validation, P17C draft-quality audit binding, and the hard-live P15/P16
...
as a complex sentinel residual caveat.
...
- P17C draft-quality audit fixture: `fixtures/eval/draft_quality_audit_p17_v1.yaml`
...
| Procedure dependency evidence | PASS | P17A selected SP suite majority gate passed; `DEPENDENCY_METADATA_INCOMPLETE` is closed with complex sentinel residual caveat recorded. |
| Validation result | PASS | P17B live pilot artifact validation is `PASSED` with no release-critical `REVIEW_REQUIRED` item. |
| Draft-quality evidence | PASS | P17C quality evidence is bound to the P17B artifact set/version and passed validation report. |
| Audit trace | PASS | P17C audit evidence links correlation id, actor, artifact refs, validation ref, quality ref, selected object refs, and evidence refs. |
...
| Policy compliance | PASS | No row-data read, procedure execution, automatic DDL/DML, PLF fallback, publish/export path, or deployment automation is included. |
...
| Stored procedure | `dbo.PCS_PY_ManageInvoiceFldSchd_PRC` | Complex sentinel; 61/63 dependencies are confirmed and two ambiguous function refs remain as residual caveats. |
...
| Table | `dbo.PEM_PRV` | Metadata-rich candidate; description evidence needs strengthening; not confirmed as selected SP dependency. |
| Table | `dbo.PEM_CTRT` | Metadata-rich candidate; description evidence needs strengthening; not confirmed as selected SP dependency. |
| View | `dbo.PCO_COM_CD_DTL_V` | View identity and definition hash evidence exist; no active caveat marker in manifest. |
| Function | `dbo.GetInitCap` | Function identity and definition hash evidence exist; no active caveat marker in manifest. |
...
| Stored procedure caveat ratio | 0.0 | Complex sentinel residual caveat is recorded as a non-blocking caveat for P17A. |
...
| Draft-quality status | Evidence bound | P17C quality evidence is tied to the P17B artifact set/version and validation report. |
| Draft artifact completeness | Scoped conditional candidate | Suitable for draft-only live pilot use, not automatic publish/export or production deployment. |

## Known Limitations And Improvements

- `COMPLEX_SENTINEL_RESIDUAL_REVIEW`: `dbo.PCS_PY_ManageInvoiceFldSchd_PRC` keeps two ambiguous function references as caveats.
...
- Publish/export is intentionally absent. Any future publish route must stay outside this draft-generation product surface.
...
- Draft-quality evidence is not bound to the latest passed validation report.
- Audit evidence lacks correlation, actor, artifact, validation, and quality context.
- The requested path requires row data, procedure execution, automatic DDL/DML, direct DB mutation, deployment automation, PLF fallback for PPM, or publish/export.
...
- P17C draft-quality evidence remains bound to the latest artifact set/version and validation report.
...
- Stakeholders understand that selected PPM object identities are metadata evidence only, not confirmed migration readiness.
