...
- API validation write action 은 `AUTH_RBAC_ENFORCEMENT=1` 일 때 verified OIDC/JWT actor 와 PLF role membership 이 있는 actor 만 수행할 수 있다.
...
## 주의
- 결과는 초안이며 검토가 필요하다.
- 기본 Web UI 는 승인/반려 화면을 제공하지 않는다. `REVIEW_REQUIRED` 는 승인 요청이 아니라 분석/evidence caveat 이다.
