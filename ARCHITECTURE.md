...
General metadata search remains a separate read-only API surface at
`GET /api/v1/metadata/search`. SP Analysis procedure-name autocomplete uses the Web
`/api/metadata/search` helper with `objectTypes=["PROCEDURE"]` against that same public
search endpoint. General search may include COLUMN identities as evidence context; columns are not
accepted as SP Analysis targets. Neither path expands the public MCP invoke allowlist or authorizes
row data, procedure execution, DDL/DML, apply, deploy, or publish.
...
- Metadata analysis은 `POST /api/v1/metadata/analyze`를 유지한다. General metadata search는 공개 `GET /api/v1/metadata/search` route를 사용하며, PROCEDURE/TABLE/COLUMN/VIEW/FUNCTION identity evidence를 반환한다. SP Analysis procedure autocomplete은 Web `/api/metadata/search` proxy가 PROCEDURE object type으로 제한해 같은 public search endpoint를 호출한다. Metadata design chat은 table design/refine run으로 분리된다.
