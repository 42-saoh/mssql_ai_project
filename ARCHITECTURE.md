...
search endpoint. Neither path expands the public MCP invoke allowlist or authorizes row data,
procedure execution, DDL/DML, apply, deploy, or publish.
...
- Metadata analysis은 `POST /api/v1/metadata/analyze`를 유지한다. General metadata search는 공개 `GET /api/v1/metadata/search` route를 사용하며, SP Analysis procedure autocomplete은 Web `/api/metadata/search` proxy가 PROCEDURE object type으로 제한해 같은 public search endpoint를 호출한다. Metadata design chat은 table design/refine run으로 분리된다.
