...
- `sp_operation_model_p41_manage_bond_v1.yaml` — P41A SP operation model renewal fixture. It records sanitized `PCO_GU_ManageBond_PRC` operation branches, statement evidence, multi-DTO blueprints, and the current single-DTO generator gap marker without raw SP text, row data, procedure execution, or new public artifact types.
...
P41A fixture 는 외부 guide 를 품질 기준으로만 사용하며 원문 본문이나 raw SP definition 을 저장하지 않는다. `DTO_DRAFT` 는 후속 P41 slice 에서 multi-file bundle 로 확장될 수 있지만, fixture 자체는 public artifact type, API, DB schema, UI, live MCP route 를 변경하지 않는다.
