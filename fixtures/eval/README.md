...
- `llm_sp_analysis_quality_p23_v1.yaml` — P23 LLM-assisted SP analysis quality suite. simple/medium/complex synthetic scenarios, deterministic facts, sanitized trace expectations, `LLM_INFERENCE`, unsupported dependency/table/function `REVIEW_REQUIRED`, `openai_fast_test` / 기본 `gpt-5-nano`, optional live `OPENAI_MODEL_FAST_TEST` override, and `production_ready: false` 를 정의한다.
