...
- P43 adds `AiGenerationFrameworkAdapter.v0.1` as an internal-only adapter contract around AI Draft Pack inventory, file-content, repair, and trace-summary stages. `BaselineResponsesFrameworkAdapter` wraps the current Responses/httpx gateway as rollback, while fake OpenAI Agents SDK and LangGraph adapters provide policy-safe replay evidence without installing framework dependencies.
...
- P44 supersedes that pilot as the active runtime direction. `FrameworkRuntimeConfig.v0.1` selects `OpenAIAgentsFrameworkAdapter` plus `LangGraphAiDraftPackOrchestrator` for OpenAI remote AI Draft Pack runs. P-GPT and emergency rollback stay on `responses_httpx`.
...
- P45 adds the optional `P44_OPENAI_AGENTS_LIVE_GATE=1` confidence gate for the adopted OpenAI Agents runtime. P46 keeps `responses_httpx` out of the OpenAI default path but retained for P-GPT and emergency rollback; deletion requires a later cleanup gate.
...
- P45 live evidence accepts only an official OpenAI Agents endpoint (`OPENAI_BASE_URL` empty or `https://api.openai.com/v1`) and an official-valid key. Custom/P-GPT-compatible endpoints remain on `responses_httpx` and are not counted as Agents SDK live evidence.
