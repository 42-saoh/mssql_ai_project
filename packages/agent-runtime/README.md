...
- P50 keeps the same public draft-pack schema but records an internal composer split for DTO inventory/content, Service content, Mapper interface content, Mapper XML content, integration quality, and repair. Workflow draft generation resolves to `openai_ai_draft_pack`; shallow branch-heavy operation evidence is blocked before model drafting, and token-only DTO/Service/Mapper/XML outputs fail deterministic validation without persisting fallback skeletons.
...
- P48 preserves evidence-ref repair, planner fallback, tool allowlists, SP source text gates, metadata/design sanitization, knowledge persistence sanitization, and `REVIEW_REQUIRED` behavior. Complex SP operation-model planning can split into `branch_plan`, `final_model`, and one validator-guided `repair` task, persisted as sanitized sidecar AgentRuns without raw failed payloads or `responses_httpx` fallback.
- The metadata design planner prompt metadata includes `toolNames` for remote structured validation.
