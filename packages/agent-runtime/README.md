...
- P44 supersedes that pilot as the active runtime direction. `FrameworkRuntimeConfig.v0.1` selects `OpenAIAgentsFrameworkAdapter` plus `LangGraphAiDraftPackOrchestrator` for OpenAI remote AI Draft Pack runs. P-GPT defaults to `responses_httpx` for compatibility, but an explicit internal `AI_GENERATION_RUNTIME=openai_agents` selection may use the OpenAI Agents SDK with an approved P-GPT-compatible endpoint and post-run P42 validation.
...
- P49 removes production exports for P43 baseline/fake framework adapter scaffolding. Equivalent fakes live under test helpers only. `responses_httpx` and `OpenAIModelGateway` remain retained for P-GPT default compatibility and explicit emergency rollback.
