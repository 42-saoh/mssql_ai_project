...
- `ModelGateway.draft_ai_java_mybatis_pack` provides the P42 `AiJavaMyBatisDraftPack.v0.1` structured-output path for draft-only Java/MyBatis file bundles. It uses the existing Responses/httpx gateway, stores only schema-valid structured output and trace hashes, and keeps `productionReady=false`.
