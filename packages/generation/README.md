...
- template version 과 output role 은 `packages/templates/artifacts/java_mybatis_registry.yaml` 에서 읽어 manifest 에 기록한다.
- 모든 draft 출력은 evidence refs, TODO, `REVIEW_REQUIRED` 경계를 포함해야 한다.
- manifest 는 sanitized input snapshot hash, policy/template version, generated file inventory, diff/review checklist, SQL risk marker 를 포함한다.
