...
- `win_git_bash.ps1` — Windows PowerShell 에서 Git Bash 를 경유해 `make`/`pnpm` 계열 명령을 실행한다. WinGet Links shim 대신 실제 WinGet package 경로와 사용 가능한 `node.exe` 경로를 PATH 앞에 붙인다.
