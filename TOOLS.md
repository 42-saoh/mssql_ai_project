...
- `mssqlMetadata`
  ...
  - P27 design-only planned tools: `get_dependency_closure`, `resolve_dependency_reference`
    는 catalog 에 inactive/read-only 로만 선언되어 있으며 handler/API/Web wiring 은 아직 없다
...
- `make test PYTEST_ARGS="tests/contract/test_p27_dependency_evidence_tooling_prompt_assets.py tests/unit/test_mcp_catalog.py tests/contract/mcp/test_tool_invocation_contract.py"` 는 P27 dependency evidence tooling design 계약과 prompt/manifest 자산을 검증한다. 새 dependency closure/resolver tool 은 inactive/read-only catalog entry 이며, raw SQL, row data, procedure execution, DDL/DML, raw definition storage, runtime workflow change, PPM-to-PLF fallback 을 허용하지 않는다.
