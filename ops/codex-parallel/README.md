...
- `prompts/*.md` — Codex 세션에 그대로 넣을 수 있는 요청 프롬프트

권장 사용 순서:
1. `PARALLEL_REQUEST_PLAN.md` 읽기
2. `PARALLEL_RUNBOOK.md` 기준으로 worktree 준비
3. `prompts/00_coordinator_baseline.md` 를 메인 코디네이터 세션에 먼저 실행
4. Wave 1 프롬프트를 각 worktree의 Codex 세션에 분산 실행
5. Wave 1 머지 후 `prompts/05_api_workflow.md`
6. 통합 후 `prompts/06_integration_eval_docs.md`
7. 마지막에 `prompts/07_final_review.md`

핵심 규칙:
- 한 트랙은 자기 `Target Paths` 바깥을 수정하지 않는다.
...
- 외부 DB 가 필요하면 worktree 별로 `.env.local` 또는 승인된 환경변수만 주입하고, repo 차원의 DB up/down 을 추가하지 않는다.
