...
중앙 통합형 단일 플랫폼 UI를 product demo 수준으로 발전시킨다. SP 분석 요청, metadata search, artifact preview, validation result, quality caveat, job status/progress 화면을 mock-first + API adapter 구조로 정리한다.
...
- quality caveat 화면은 실제 승인/배포 실행이 아니라 decision preview/recording UI 경계로 둔다.
